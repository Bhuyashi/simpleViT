...
parser.add_argument("--pgd", default=False, type=bool, help="Train with advaersarial samples")
...
def main(args):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint = torch.load(args.resume) #map_location = torch.device('cpu')) # FOr CPU pupose
...
def train(train_loader, model, criterion, optimizer, scheduler, epoch, args, writer, adversary=False):
    """Training the model"""
    ...
    attacker = default_attack(criterion)
    if adversary:
        print("Training with adversarial samples")
    else:
        print("Training without adversarial samples")
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        toss = random.random()
        if adversary and toss < 0.2:
            attacker.change_state(num_steps= random.randint(5, 20), epsilon=random.uniform(0.05, 0.15))
            adversary_input = attacker.perturb(model, input)
            input = torch.cat([input, adversary_input], dim=0)
            target = torch.cat([target, target], dim=0)

        # zero grad
        # after adversarial samples; to zero out model gradients
        optimizer.zero_grad()
